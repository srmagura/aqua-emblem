// Generated by CoffeeScript 1.12.2
(function() {
  _enc.UnitAction = (function() {
    function UnitAction(ui, unit) {
      this.ui = ui;
      this.unit = unit;
    }

    UnitAction.prototype.doAction = function(action, callback, delta) {
      var afterDelay, afterFadeIn, boxEl, css, left0, tw;
      this.action = action;
      this.callback = callback;
      this.delta = delta;
      this.container = $('<div></div>');
      this.container.addClass('unit-action-container').appendTo('.canvas-container');
      boxEl = $('.sidebar .unit-info').clone().addClass('blue-box');
      this.container.append(boxEl);
      this.box = new _chapterui.UnitInfoBox(this.ui, boxEl);
      this.box.init(this.unit, false, false);
      this.box.show();
      tw = this.ui.tw;
      left0 = (this.unit.pos.j + .5) * tw - this.ui.origin.j;
      css = {};
      css.left = left0 - boxEl.width() / 2;
      css.top = (this.unit.pos.i + 1) * tw + 5 - this.ui.origin.i;
      this.container.css(css);
      if (this.action.getMessageEl != null) {
        this.message = this.action.getMessageEl();
        this.message.appendTo(this.container);
        if (this.unit.team instanceof _team.PlayerTeam) {
          this.message.addClass('blue-box');
        } else {
          this.message.addClass('red-box');
        }
      }
      afterFadeIn = (function(_this) {
        return function() {
          setTimeout(afterDelay, _this.delay * 4 / 3);
          if (_this.action instanceof _skill.Skill) {
            _this.unit.mp -= _this.action.mp;
          }
          if ((_this.delta != null) && 'hp' in _this.delta) {
            _this.unit.addHp(_this.delta.hp);
          }
          return _this.box.init(_this.unit, true, false);
        };
      })(this);
      afterDelay = (function(_this) {
        return function() {
          _this.container.fadeOut(_this.delay / 3);
          return _this.message.fadeOut(_this.delay / 3, _this.callback);
        };
      })(this);
      this.delay = _enc.DELAY / this.ui.speedMultiplier;
      return this.message.fadeIn(this.delay / 3, afterFadeIn);
    };

    return UnitAction;

  })();

}).call(this);
