...
(function() {
  ...
  _enc.Battle = (function(_super) {
    ...
    Battle.prototype.calcBattleStats = function() {
      var defCanAttack, i, unit, _i, _j, _k, _l, _len, _m, _ref, _ref1, _ref2, _ref3, _ref4;
      ...
      this.turns = [];
      for (i = _i = 1, _ref = this.atk.equipped.nAttackMultiplier; 1 <= _ref ? _i <= _ref : _i >= _ref; i = 1 <= _ref ? ++_i : --_i) {
      ...
      if (defCanAttack) {
        for (i = _j = 1, _ref1 = this.def.equipped.nAttackMultiplier; 1 <= _ref1 ? _j <= _ref1 : _j >= _ref1; i = 1 <= _ref1 ? ++_j : --_j) {
          this.turns.push(this.def);
        }
      }
      if (this.atk.attackSpeed - 4 > this.def.attackSpeed && this.atk.equipped.hasUses(2)) {
        for (i = _k = 1, _ref2 = this.atk.equipped.nAttackMultiplier; 1 <= _ref2 ? _k <= _ref2 : _k >= _ref2; i = 1 <= _ref2 ? ++_k : --_k) {
          this.turns.push(this.atk);
        }
      } else if (defCanAttack && this.def.attackSpeed - 4 > this.atk.attackSpeed && this.def.equipped.hasUses(2)) {
        for (i = _l = 1, _ref3 = this.def.equipped.nAttackMultiplier; 1 <= _ref3 ? _l <= _ref3 : _l >= _ref3; i = 1 <= _ref3 ? ++_l : --_l) {
          this.turns.push(this.def);
        }
      }
      this.nTurns = {
