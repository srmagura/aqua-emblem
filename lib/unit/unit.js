...
(function() {
  ...
  _unit.Unit = (function() {
    function Unit(attr) {
      this.updateLunge = bind(this.updateLunge, this);
      var key, value;
      for (key in attr) {
        value = attr[key];
        this[key] = value;
    ...
    Unit.prototype.onNewTurn = function() {
      var i, j, k, len, len1, ref, ref1, status, toRemove;
      ref = this.statuses;
    ...
    Unit.prototype.onEnemyTurn = function() {
      var j, len, ref, results, status;
      ref = this.statuses;
      results = [];
      for (j = 0, len = ref.length; j < len; j++) {
        status = ref[j];
        results.push(status.onEnemyTurn(this));
      }
      return results;
    };

    Unit.prototype.addStatus = function(status) {
      var doAdd, i, j, k, len, len1, other, ref, toRemove;
      doAdd = true;
      toRemove = [];
      ref = this.statuses;
    ...
    Unit.prototype.fillInStartStats = function(startStats) {
      ...
      for (stat in startStats) {
        ...
        if (!(stat in this.startStats)) {
          results.push(this.startStats[stat] = value);
        } else {
          results.push(void 0);
        }
      }
    ...
    Unit.prototype.calcStats = function(dryRun) {
      var increment, j, len, obj, ref, ref1, rounded, stat, value;
      if (dryRun == null) {
        dryRun = false;
      }
      increment = {};
    ...
    Unit.prototype.updateStats = function() {
      ...
      for (stat in this.baseStats) {
        results.push(this.updateStat(stat));
      }
      return results;
    };

    Unit.prototype.updateStat = function(stat) {
      var j, len, ref, status, value;
